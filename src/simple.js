// function getMinSortNum(dataArr) {
...
function getMinSortNum(dataArr, lastMin) {
    const rawArr = dataArr.slice();
    const min = Math.min.apply(null, rawArr);
    const tag = rawArr.indexOf(min);
    if (dataArr.length === 0) return 0;
    if (dataArr.length === 1) return lastMin === min ? 0 : 1;
    if (lastMin === void 0 && JSON.stringify(dataArr) === JSON.stringify(rawArr.slice().sort((a, b) => a - b))) return 0;
